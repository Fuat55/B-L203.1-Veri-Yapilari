/*
Fuat ŞENGÜL, 20061108, Bilgisayar Mühendisliği
Ödev-6: B+ Tree

Yazdır() fonksiyonu yapılamadı.
*/
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
void siralama()
{
    ...
    for (size_t x = 0; x < (N - 1); x++)
    {
        ...
        {
            if (OgrListe[y][1] < OgrListe[y + 1][1])
            {
                tpuan = OgrListe[y][1];
                tnumara = OgrListe[y][0];

                OgrListe[y][1] = OgrListe[y + 1][1];
                OgrListe[y][0] = OgrListe[y + 1][0];

                OgrListe[y + 1][1] = tpuan;
                OgrListe[y + 1][0] = tnumara;
            }
            else if (OgrListe[y][1] == OgrListe[y + 1][1])
            {
                if (OgrListe[y][0] > OgrListe[y + 1][0])
                {
                    tpuan = OgrListe[y][1];
                    tnumara = OgrListe[y][0];
...
void kayitlariEkle()
{
    ...
    {
        rootMain = insert(rootMain, i, OgrListe[i][1]);
    }
}
void guncelle(int numara, int yeniPuan)
{

    for (size_t i = 0; i < N; i++)
    {
        if (OgrListe[i][0] == numara)
        {
            OgrListe[i][1] = yeniPuan;
            break;
        }
    }
    destroy_tree(rootMain);
    siralama();
    kayitlariEkle();
}

int bulOgrenciIndex(int numara)
{
    size_t i;
    for (i = 0; i < N; i++)
    {
...
int main()
{
    ...
    rootMain = delete (rootMain, bulOgrenciIndex(18060311));
    printf("\n");
    print_tree(rootMain);
}